...
if [[ ! -x "$TSX_BIN" ]]; then
  echo "خطأ: $TSX_BIN غير موجود — نفّذ npm install أولاً."
  exit 1
fi
...
# لا تستخدم: pm2 start npm --cwd ... (قد يشغّل ./npm داخل المشروع بالخطأ)
PORT="$OBADA_API_PORT" NODE_ENV=production pm2 start "$TSX_BIN" \
  --name "$OBADA_PM2_NAME" \
  --cwd "$ROOT" \
  -- server/src/index.ts
pm2 save
