        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 space-y-3">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
              <div className="rounded-lg bg-slate-50 p-3"><div className="text-xs text-slate-500">نوع الخامة</div><div className="font-bold">{(meta as { fabricFamily?: string; importMaterialName?: string })?.importMaterialName ?? (meta as { fabricFamily?: string })?.fabricFamily ?? meta?.materialName ?? '—'}</div></div>
              <div className="rounded-lg bg-slate-50 p-3"><div className="text-xs text-slate-500">كود التصميم</div><div className="font-bold">{(meta as { designCodeFromFile?: string })?.designCodeFromFile ?? meta?.materialName ?? '—'}</div></div>
              ...
              <div className="rounded-lg bg-slate-50 p-3"><div className="text-xs text-slate-500">عدد الأتواب</div><div className="font-bold">{preview.rowCount}{(meta as { declaredRollCount?: number })?.declaredRollCount ? ` / ${(meta as { declaredRollCount?: number }).declaredRollCount} بالملف` : ''}</div></div>
              <div className="rounded-lg bg-slate-50 p-3"><div className="text-xs text-slate-500">إجمالي الأمتار</div><div className="font-bold">{preview.totalLengthM.toLocaleString('ar-EG')}{(meta as { declaredTotalLength?: number })?.declaredTotalLength ? ` / ${Number((meta as { declaredTotalLength?: number }).declaredTotalLength).toLocaleString('ar-EG')} بالملف` : ''}</div></div>
